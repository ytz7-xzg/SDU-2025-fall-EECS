...
################
    ...
    def addCurrent(self, current, node, sign):
        if node not in self.nodes:
            self.nodes[node] = []
        self.nodes[node].append((float(sign),current))
...
################
    ...
    def addCurrents(self, currents):
        for current in currents:
            ...
            node_name = current[1]
            sign = current[2]
            self.addCurrent(current_name, node_name, sign)
...
################
    ...
    def getKCLEquations(self, gnd):
        ...
        equations.append(le.Equation([1.0], [gnd], 0.0))

        for node_name, terms in self.nodes.items():
            ...
            if node_name != gnd:
                ...
                if coeffs:
                    kcl_eq = le.Equation(coeffs, varNames, 0.0)
...
class Resistor(Component):
    ...
    def getEquation(self):
        return le.Equation([1, -1, -self.r],[self.n1, self.n2, self.current],0.0)
...
class OpAmp(Component):
    ...
    def getEquation(self):
        return le.Equation([self.K,-self.K,-1],[self.nPlus, self.nMinus, self.nOut],0.0)
